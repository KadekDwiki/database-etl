...
def extract_data(conn):
   query = "SELECT * FROM pengarang_transformed"  # Gantilah nama tabel dengan tabel yang sesuai
...
def load_data_to_warehouse(conn, df):
   ...
   create_table_query = """
   CREATE TABLE IF NOT EXISTS dim_pengarang (
      id INT PRIMARY KEY,
      nama VARCHAR(255),
      telp VARCHAR(12),
      alamat TINYTEXT
   ...
   for _, row in df.iterrows():
      ...
      if existing_data:
         ...
         insert_query = """
         INSERT INTO dim_pengarang (id, nama, telp, alamat)
         VALUES (%s, %s, %s, %s)
